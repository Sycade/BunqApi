...
namespace Sycade.BunqApi.Model.MonetaryAccounts
{
    public class MonetaryAccountBank : MonetaryAccount
    {
        ...
        {
            return await ApiClient.Payments.CreateAsync(Id, to, amount, description, session);
        }

        public async Task<Id> CreatePaymentAsync(MonetaryAccountBank to, Amount amount, string description, Session session)
        {
            return await ApiClient.Payments.CreateAsync(Id, to.Aliases.First(a => a.Type == AliasType.IBAN), amount, description, session);
        }
    }
}
